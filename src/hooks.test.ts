...
describe('gji new --reuse with afterCreate hook', () => {
  it('runs afterCreate hook in an already-existing worktree when --reuse is passed', async () => {
    // Given a worktree that was previously created, plus an afterCreate hook.
    const repoRoot = await createRepository();
    const branchName = 'feature/already-exists';
    const worktreePath = resolveWorktreePath(repoRoot, branchName);

    // Create the worktree first (no hook configured yet).
    await runCli(['new', branchName], { cwd: repoRoot });

    // Now configure an afterCreate hook.
    const markerFile = join(worktreePath, '.reuse-hook-ran');
    await writeFile(
      join(repoRoot, '.gji.json'),
      JSON.stringify({ hooks: { afterCreate: `touch "${markerFile}"` } }),
      'utf8',
    ...
    // When gji new --reuse is run for the same branch.
    const stdout: string[] = [];
    const result = await runCli(['new', '--reuse', branchName], {
      cwd: repoRoot,
      stdout: (c) => stdout.push(c),
    });

    // Then the command succeeds, outputs the existing path, and the hook ran.
    expect(result.exitCode).toBe(0);
    expect(stdout.join('')).toBe(`${worktreePath}\n`);
    await expect(readFile(markerFile)).resolves.toBeDefined();
  });

  it('creates the worktree normally when --reuse is passed but the path does not exist yet', async () => {
    // Given a repo and a branch that has never been created.
    const repoRoot = await createRepository();
    const branchName = 'feature/brand-new';
    const worktreePath = resolveWorktreePath(repoRoot, branchName);
    const markerFile = join(worktreePath, '.hook-ran');
    ...
    // When gji new --reuse is run for a non-existing branch.
    const stdout: string[] = [];
    const result = await runCli(['new', '--reuse', branchName], {
      cwd: repoRoot,
      stdout: (c) => stdout.push(c),
    ...
    // Then the worktree is created normally and the hook still runs.
    expect(result.exitCode).toBe(0);
    expect(stdout.join('')).toBe(`${worktreePath}\n`);
    await expect(readFile(markerFile)).resolves.toBeDefined();
  });

  it('runs afterCreate hook in headless mode when --reuse is passed and worktree exists', async () => {
    // Given a pre-existing worktree and a hook – this is the AI-agent scenario.
    const repoRoot = await createRepository();
    const branchName = 'feature/agent-reuse';
    const worktreePath = resolveWorktreePath(repoRoot, branchName);

    await runCli(['new', branchName], { cwd: repoRoot });

    const markerFile = join(worktreePath, '.agent-hook-ran');
    await writeFile(
      join(repoRoot, '.gji.json'),
      JSON.stringify({ hooks: { afterCreate: `touch "${markerFile}"` } }),
      'utf8',
    ...
    const originalNoTui = process.env.GJI_NO_TUI;
    process.env.GJI_NO_TUI = '1';

    try {
      // When an AI agent runs gji new --reuse in headless mode.
      const result = await runCli(['new', '--reuse', branchName], { cwd: repoRoot });

      // Then the command succeeds and the hook ran.
      expect(result.exitCode).toBe(0);
      await expect(readFile(markerFile)).resolves.toBeDefined();
    } finally {
      if (originalNoTui === undefined) {
        delete process.env.GJI_NO_TUI;
      } else {
        process.env.GJI_NO_TUI = originalNoTui;
      }
    }
  });
});
