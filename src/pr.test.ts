...
describe("gji pr", () => {
	...
	it("accepts a GitHub PR URL", async () => {
		...
		const result = await runCli(
			["pr", "https://github.com/owner/repo/pull/555"],
			{
				cwd: repoRoot,
			},
		);
	...
	it("fetches a GitLab merge request ref from origin and creates a linked worktree", async () => {
		...
		const result = await runCli(
			["pr", "https://gitlab.com/owner/repo/-/merge_requests/556"],
			{
				cwd: repoRoot,
			},
		);
	...
	it("fetches a Bitbucket pull request ref from origin and creates a linked worktree", async () => {
		...
		const result = await runCli(
			["pr", "https://bitbucket.org/owner/repo/pull-requests/557"],
			{
				cwd: repoRoot,
			},
		);
