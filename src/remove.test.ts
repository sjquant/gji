...
describe("gji remove", () => {
	...
	it("shows recency, path, and dirty state in the destructive picker", async () => {
		...
		try {
			...
			expect(capturedEntry?.label).toContain("last used: 4m ago");
			expect(capturedEntry?.label).toContain("remove-safety-labels");
		...
		const runRemoveCommand = createRemoveCommand({
			...
			promptForWorktree: async (worktrees) => {
				expect(worktrees.map((worktree) => worktree.branch).sort()).toEqual(
					[removeBranch, "feature/keep", null].sort(),
				);
	...
	describe("--json output", () => {
		it("emits { branch, path, deleted: true } to stdout on success", async () => {
			...
			const runRemoveCommand = createRemoveCommand({
				confirmRemoval: async () => {
					throw new Error("confirmation must not be called in --json mode");
				},
			});
