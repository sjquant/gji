...
describe("gji status", () => {
	...
	it('serializes stale upstream as { kind: "stale" } in JSON output', async () => {
		...
		const json = JSON.parse(stdout.join("")) as {
			worktrees: Array<{ branch: string; upstream: unknown }>;
		};
		...
		expect(mainEntry?.upstream).toEqual({
			kind: "tracked",
			ahead: 0,
			behind: 0,
		});
	...
	it("prints stable structured JSON with repository metadata and upstream state", async () => {
		...
		const trackedWorktreePath = await addLinkedWorktree(
			repoRoot,
			trackedBranch,
		);
		const untrackedWorktreePath = await addLinkedWorktree(
			repoRoot,
			untrackedBranch,
		);
		const detachedWorktreePath = `${repoRoot}-detached`;
		...
		await writeFile(
			join(untrackedWorktreePath, "dirty.txt"),
			"dirty\n",
			"utf8",
		);
