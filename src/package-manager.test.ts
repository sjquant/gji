...
describe('detectPackageManager', () => {
  ...
  it('returns null for a non-existent directory', async () => {
    expect(await detectPackageManager('/tmp/gji-nonexistent-dir-xyz')).toBeNull();
  });
  ...
  it('detects renv.lock', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'renv', installCommand: "Rscript -e 'renv::restore()'" });
  });
  ...
  it('detects Cargo.lock', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'cargo', installCommand: 'cargo build' });
  });
  ...
  it('detects go.sum', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'go', installCommand: 'go mod download' });
  });
  ...
  it('detects Gemfile.lock', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'bundler', installCommand: 'bundle install' });
  });
  ...
  it('detects composer.lock', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'composer', installCommand: 'composer install' });
  });
  ...
  it('detects pubspec.lock', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'dart', installCommand: 'dart pub get' });
  });
  ...
  it('detects pom.xml', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'maven', installCommand: 'mvn install' });
  });

  it('detects gradlew and uses wrapper command', async () => {
    const dir = await makeDir();
    await touch(dir, 'gradlew');
    expect(await detectPackageManager(dir)).toEqual({ name: 'gradle', installCommand: './gradlew build' });
  });
  ...
  it.each([
    ...
    const dir = await makeDir();
    await touch(dir, file);
    expect(await detectPackageManager(dir)).toEqual({ name: 'gradle', installCommand: 'gradle build' });
  });
  ...
  it('detects build.sbt', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'sbt', installCommand: 'sbt compile' });
  });
  ...
  it.each([
    ...
    const dir = await makeDir();
    await touch(dir, file);
    expect(await detectPackageManager(dir)).toEqual({ name: 'dotnet', installCommand: 'dotnet restore' });
  });
  ...
  it('detects Package.swift', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'swift', installCommand: 'swift package resolve' });
  });
  ...
  it('detects stack.yaml', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'stack', installCommand: 'stack build' });
  });

  it('detects cabal.project', async () => {
    const dir = await makeDir();
    await touch(dir, 'cabal.project');
    expect(await detectPackageManager(dir)).toEqual({ name: 'cabal', installCommand: 'cabal install --only-dependencies' });
  });
  ...
  it('detects dune-project', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'dune', installCommand: 'dune build' });
  });
  ...
  it('detects Manifest.toml', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'julia', installCommand: "julia --project -e 'using Pkg; Pkg.instantiate()'" });
  });
  ...
  it('detects *.nimble via glob', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'nimble', installCommand: 'nimble install' });
  });
  ...
  it('detects shard.yml', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'shards', installCommand: 'shards install' });
  });
  ...
  it('detects cpanfile', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'cpanm', installCommand: 'cpanm --installdeps .' });
  });
  ...
  it('detects build.zig.zon', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'zig', installCommand: 'zig build' });
  });
  ...
  it('prefers flake.nix over shell.nix', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'nix', installCommand: 'nix develop' });
  });
  ...
  it('detects terraform.lock.hcl', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'terraform', installCommand: 'terraform init' });
  });
  ...
  it('prefers pnpm over yarn when both lockfiles are present', async () => {
    ...
    expect(await detectPackageManager(dir)).toEqual({ name: 'pnpm', installCommand: 'pnpm install' });
  });
