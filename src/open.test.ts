...
describe("gji open", () => {
	...
	it("places the current worktree first in the selector", async () => {
		...
		let capturedFirst:
			| { branch: string | null; isCurrent: boolean }
			| undefined;
		const runOpenCommand = createOpenCommand({
			promptForWorktree: async (worktrees) => {
				capturedFirst = {
					branch: worktrees[0].branch,
					isCurrent: worktrees[0].isCurrent,
				};
				return worktreeA;
			},
	...
	it("aborts when the worktree prompt is cancelled", async () => {
		...
		const runOpenCommand = createOpenCommand({
			...
			spawnEditor: async () => {
				throw new Error("spawn must not be called after abort");
			},
		});
	...
	it("opens a linked worktree by branch name without prompting", async () => {
		...
		const runOpenCommand = createOpenCommand({
			promptForWorktree: async () => {
				throw new Error("prompt must not be called when branch is given");
			},
			spawnEditor: async (cli, args) => {
				spawned.push({ cli, args });
			},
	...
	it("opens the current worktree in the specified editor", async () => {
		...
		const runOpenCommand = createOpenCommand({
			promptForWorktree: async (worktrees) =>
				worktrees.find((w) => w.isCurrent)?.path ?? null,
			spawnEditor: async (cli, args) => {
				spawned.push({ cli, args });
			},
		});
	...
	it("exits 1 when the requested branch has no worktree", async () => {
		...
		const runOpenCommand = createOpenCommand({
			spawnEditor: async () => {
				throw new Error("spawn must not be called when branch is not found");
			},
		});
	...
	it("exits 1 when no editor is detected or configured", async () => {
		...
		const runOpenCommand = createOpenCommand({
			...
			spawnEditor: async () => {
				throw new Error("spawn must not be called when no editor is found");
			},
		});
	...
	it("auto-selects the only detected editor without prompting", async () => {
		...
		const runOpenCommand = createOpenCommand({
			...
			promptForEditor: async () => {
				throw new Error("editor prompt must not be called with one editor");
			},
			promptForWorktree: async (worktrees) => worktrees[0]?.path ?? null,
			spawnEditor: async (cli, args) => {
				spawned.push({ cli, args });
