...
export async function detectPackageManager(repoRoot: string): Promise<PackageManager | null> {
...
async function matchesExact(repoRoot: string, signals: string[]): Promise<boolean> {
...
async function matchesGlob(repoRoot: string, patterns: string[]): Promise<boolean> {
  let files: string[];
