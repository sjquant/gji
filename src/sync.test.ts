...
describe("gji sync", () => {
	it("syncs the current linked worktree onto the latest default branch", async () => {
		...
		const untouchedWorktreePath = await addLinkedWorktree(
			repoRoot,
			untouchedBranch,
		);
		const stdout: string[] = [];
		...
		await expect(
			pathExists(join(targetWorktreePath, "base-update.txt")),
		).resolves.toBe(true);
		await expect(
			pathExists(join(untouchedWorktreePath, "base-update.txt")),
		).resolves.toBe(false);
	});
	...
	it("syncs every worktree when --all is provided", async () => {
		...
		await expect(
			pathExists(join(firstWorktreePath, "all-update.txt")),
		).resolves.toBe(true);
		await expect(
			pathExists(join(secondWorktreePath, "all-update.txt")),
		).resolves.toBe(true);
	});
	...
	it("refuses to sync a dirty worktree", async () => {
		...
		expect(stderr.join("")).toBe(
			`Cannot sync dirty worktree: ${worktreePath}\n`,
		);
	});
	...
	it("uses the remote default branch even when the repository root is on another branch", async () => {
		...
		expect(stdout.join("")).toBe(`${targetWorktreePath}\n`);
		await expect(
			pathExists(join(targetWorktreePath, "remote-default.txt")),
		).resolves.toBe(true);
	});
	...
	it("skips detached worktrees during sync --all", async () => {
		...
		const featureWorktreePath = await addLinkedWorktree(
			repoRoot,
			featureBranch,
		);
		...
		await expect(
			pathExists(join(featureWorktreePath, "skip-detached.txt")),
		).resolves.toBe(true);
		await expect(
			pathExists(join(detachedWorktreePath, "skip-detached.txt")),
		).resolves.toBe(false);
	});
	...
	it("refuses to sync when the current worktree is detached", async () => {
		...
		expect(result.exitCode).toBe(1);
		expect(stderr.join("")).toBe(
			`Cannot sync detached worktree: ${detachedWorktreePath}\n`,
		);
	});
	...
	it("uses repo-local sync config for remote and default-branch resolution", async () => {
		...
		expect(stdout.join("")).toBe(`${targetWorktreePath}\n`);
		await expect(
			pathExists(join(targetWorktreePath, "configured-sync.txt")),
		).resolves.toBe(true);
	});
});
...
describe("gji sync --json", () => {
	it("emits { updated } to stdout on success", async () => {
		...
		expect(output.updated[0]).toEqual({
