...
_gji_completion() {
  ...
  case "$command_name" in
    ...
    open)
      ...
      ;;
    go|jump)
      COMPREPLY=( $(compgen -W "$(__gji_worktree_branches) --root --print --json --help" -- "$cur") )
      ;;
    root)
      COMPREPLY=( $(compgen -W "--print --help" -- "$cur") )
      ;;
...
case "\${words[2]}" in
  ...
  open)
    ...
    ;;
  go|jump)
    _arguments '--root[navigate to the main repository root]' '--print[print the resolved worktree path explicitly]' '--json[emit JSON for an existing worktree destination]' '2:branch:->worktrees'
    ;;
  root)
