...
export async function maybeRunInstallPrompt(
	...
	const detect =
		dependencies.detectInstallPackageManager ?? detectPackageManager;
	...
	const prompt =
		dependencies.promptForInstallChoice ?? defaultPromptForInstallChoice;
	...
	const writeGlobalKey =
		dependencies.writeGlobalRepoConfigKey ?? defaultWriteGlobalRepoConfigKey;
...
async function loadExistingGlobalRepoHooks(
	repoRoot: string,
	...
	const perRepo = isPlainObject(repos[repoRoot])
		? (repos[repoRoot] as Record<string, unknown>)
		: {};

	return isPlainObject(perRepo.hooks) ? perRepo.hooks : {};
}

async function defaultPromptForInstallChoice(
	pm: PackageManager,
): Promise<InstallChoice | null> {
