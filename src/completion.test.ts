...
describe('gji completion', () => {
  it('prints zsh completions explicitly', async () => {
    ...
    expect(stdout.join('')).toContain("'warp:jump to any worktree across all known repos'");
    expect(stdout.join('')).toContain("'2:shell:(bash fish zsh)'");
    expect(stdout.join('')).toContain("'--force[remove and recreate the worktree if the target path already exists]'");
    expect(stdout.join('')).toContain("'--editor[editor CLI to use with --open (code, cursor, zed, …)]:editor:'");
    expect(stdout.join('')).toContain("'--workspace[generate a .code-workspace file before opening (VS Code / Cursor / Windsurf)]'");
    expect(stdout.join('')).toContain("'(-n --new)'{-n,--new}'[create a new worktree in a registered repo]:branch:'");
    ...
    expect(stdout.join('')).toContain("_values 'config key' branchPrefix editor hooks installSaveTarget shellIntegration skipInstallPrompt syncDefaultBranch syncFiles syncRemote worktreePath repos");
...
print -r -- "\${_comps[gji]-unset}"`,
  ...
  it('auto-detects the shell from SHELL when no shell is provided', async () => {
    ...
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_use_subcommand' -a 'back'");
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_use_subcommand' -a 'open'");
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_use_subcommand' -a 'jump'");
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_use_subcommand' -a 'warp'");
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_seen_subcommand_from new' -l force");
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_seen_subcommand_from new' -l editor -r");
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_seen_subcommand_from open' -l workspace");
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_seen_subcommand_from go jump' -a '(__gji_worktree_branches)'");
    expect(stdout.join('')).toContain("complete -c gji -n '__fish_seen_subcommand_from warp' -s n -l new");
    ...
    expect(stdout.join('')).toContain("complete -c gji -n '__gji_should_complete_config_key' -a 'editor' -d 'config key'");
    expect(stdout.join('')).toContain("complete -c gji -n '__gji_should_complete_config_key' -a 'shellIntegration' -d 'config key'");
    expect(stdout.join('')).toContain("complete -c gji -n '__gji_should_complete_config_key' -a 'worktreePath' -d 'config key'");
    expect(stdout.join('')).toContain("complete -c gji -n '__gji_should_complete_config_key' -a 'repos' -d 'config key'");
