...
For sequential feature work where you want to avoid another worktree and its
dependency setup, use branch-only mode:

```bash
gji new feature/auth-tests --branch-only
```

This requires a clean current worktree, fetches the configured default branch,
and checks out the new branch there without sharing a checkout of the base
branch with another worktree. Use `--no-fetch` when the local default branch
...
- `gji go` to jump to any worktree by branch name or interactive selection; its chooser starts in the current repository and `Tab` includes all registered repositories
