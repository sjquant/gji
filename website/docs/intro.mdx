...
- Navigate to an existing worktree across registered repos.
- Navigate to a worktree in another repo from the same command entry point.
...
- Read the [comparison guide](./comparison) if you are deciding between `gji`, raw `git worktree`, `lazygit`, or `ghq`.
- Install [shell integration](./shell-integration) so directory handoff works correctly.
- Copy the [quick start](./quick-start) into your shell history.
- Use the [daily workflow](./daily-workflow) guide once the basics are installed.
