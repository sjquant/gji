...
`--branch-only` requires a clean worktree, switches to the configured default
branch with `--ignore-other-worktrees`, fast-forwards it from the configured
remote, and switches to the new branch. It does not create a worktree or run
dependency bootstrap. Pass `--no-fetch` to skip the remote update.
