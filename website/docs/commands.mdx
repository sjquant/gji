...
| `gji new [branch] [--branch-only] [--from-current] [--detached] [--take] [--copy] [--task <description>] [--force] [--open] [--editor <cli>] [--dry-run] [--json]` | Create a branch and worktree, or update the default branch and create one in the current worktree with `--branch-only`. |
...
If installing dependencies for every task is unnecessary, reuse the current
worktree instead:

```bash
gji new feature/auth-tests --branch-only
```

`--branch-only` requires a clean worktree, fetches the configured default
branch, and checks out the new branch from that remote ref without creating a
worktree or running dependency bootstrap. Pass `--no-fetch` to use the local
