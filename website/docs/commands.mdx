...
| `gji warp [branch] [--print] [--json]` | Jump to any known repo worktree. |
...
`gji go` resolves destinations in one predictable order: a worktree in the current repository, a worktree in another registered repository, an existing local branch, a fetched remote branch, and finally a pull-request reference.
...
Use `--json` only for already-existing destinations; it never creates a worktree or fetches a PR. Use `gji new` when you want to create a worktree explicitly:
...
Use `gji warp` when branch intent matters more than the repo directory you happen to be inside. `repo/branch` disambiguates across registered repos; use `gji new` to create a worktree explicitly.
