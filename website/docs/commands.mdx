...
Running `gji` without a subcommand opens the repository discovery hub. It
summarizes the current repository and registered repositories, their active
worktrees, task metadata, health, and open pull requests. Use `gji --json` for
dashboard, IDE, fzf, and other ecosystem integrations; this projection is
additive to the existing `ls --json` and `status --json` contracts.

## Everyday commands

| Command | What it does |
| --- | --- |
| `gji [--json]` | Show the repository/worktree discovery hub, optionally as structured JSON. |
