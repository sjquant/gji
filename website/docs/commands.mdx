...
It recommends a next worktree, separates worktrees needing attention, and
collapses quiet worktrees. Use `gji --attention` to show only actionable
worktrees, or `gji --json` for dashboard, IDE, fzf, and other ecosystem
...
| --- | --- |
| `gji [--attention] [--json]` | Show the action-oriented repository/worktree hub; optionally limit human output to actionable worktrees or emit structured JSON. |
