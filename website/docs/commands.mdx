...
gji go --new fix/copy-regression
```

Use `--json` only for already-existing destinations; it never creates a worktree or fetches a PR.

Use `--new` to create a worktree in the current repository, optionally supplying the branch as the option value or positional argument.
