...
| `gji back [--list] [--print]` | Return to the previously visited worktree. |
...
`gji back` behaves like `cd -` for worktrees. Run it once to return to wherever you were before. Run it again to toggle back. Use `--list` to see the full navigation history:
