...
`dependencyBuildCommand` overrides Cargo's default `cargo check`. npm is always install-only, and a sync-file failure stops dependency repair and `after-create` hooks. Dry-run output reports the effective strategy, including repair-only cases where a seed is not compatible.
