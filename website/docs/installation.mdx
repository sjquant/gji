...
On supported macOS and Linux filesystems, `gji new` clones configured directories before `syncFiles`. Dependency adapters then run authoritative repair before `afterCreate`: Yarn, pnpm, uv, and Cargo can reuse CoW seeds, while npm remains install-only. Unsupported filesystems never fall back to ordinary copying; they run repair/install from an empty target. Use `gji new --dry-run` to inspect the estimated source sizes and bootstrap strategy.
