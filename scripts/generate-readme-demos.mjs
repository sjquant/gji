...
function generateDemo(demo) {
	...
	const firstFrameSvgPath = renderFrameSvg(
		castV2Path,
		frameSvgDir,
		demo,
		0,
		capturePlan.frameCaptureTimesMs[0],
	);
	...
	writeFileSync(
		concatPath,
		renderConcatManifest(framePaths, capturePlan.frameDurationsS),
	);
	...
	return [
		{
			...
			steps: [
				...
				{
					text: "Saved working directory and index state On main: wip",
					pauseAfterSeconds: 0.5,
				},
				{
					text: prompt("git switch teammate/review-1234"),
					pauseAfterSeconds: 0.55,
				},
				{
					text: "Switched to branch 'teammate/review-1234'",
					pauseAfterSeconds: 0.5,
				},
				{ text: prompt("npm install"), pauseAfterSeconds: 0.55 },
				{
					text: "added 412 packages, and audited 413 packages in 4s",
					pauseAfterSeconds: 0.45,
				},
				...
				{
					text: "CONFLICT (content): Merge conflict in package.json",
		...
		},
		{
			...
			steps: [
				...
				{
					text: prompt("pwd", "~/code/worktrees/gji/pr-1234"),
					pauseAfterSeconds: 0.45,
				},
				{
					text: "/Users/me/code/worktrees/gji/pr-1234",
					pauseAfterSeconds: 0.5,
				},
				{
					text: prompt("gji ls", "~/code/worktrees/gji/pr-1234"),
					pauseAfterSeconds: 0.5,
				},
				{ text: "main      ~/code/gji", pauseAfterSeconds: 0.35 },
				{
					text: "pr-1234   ~/code/worktrees/gji/pr-1234",
					pauseAfterSeconds: 1.3,
				},
			],
		},
	];
...
function renderFrameSvg(castV2Path, frameSvgDir, demo, index, captureTimeMs) {
	const frameSvgPath = join(
		frameSvgDir,
		`frame-${String(index).padStart(3, "0")}.svg`,
	);
...
function captureFrames(
	...
	for (const [index, frameCaptureTimeMs] of frameCaptureTimesMs.entries()) {
		...
		const frameSvgPath = renderFrameSvg(
			castV2Path,
			frameSvgDir,
			demo,
			index,
		...
		const frameHtmlPath = renderFramePage(
			frameSvgDir,
			frameSvgPath,
			scaledViewport,
		);
...
function renderFramePage(frameSvgDir, frameSvgPath, scaledViewport) {
	const frameHtmlPath = join(
		frameSvgDir,
		`${basename(frameSvgPath, ".svg")}.html`,
	);
